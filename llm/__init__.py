...
公共出口（调用方只从这里 import，内部文件拆分不影响外部）：
  ...
  - OpenAICompatibleChat   OpenAI 兼容客户端（router.py）
  - AnthropicCompatibleChat Anthropic Messages 协议客户端，Kimi 编程订阅端点（router.py）
...
from llm.router import AnthropicCompatibleChat, LLMRoute, ModelRouter, OpenAICompatibleChat
