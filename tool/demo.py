...
from user_profile import (
    ProfileRequest, ProfileResponse, ProfileData, UserProfile,
  )
  
all_profiles = {
  "client007": UserProfile (
    behaviors={
...
class UserServerHttpClient:
  ...
  async def query_user_profile(self, uid: str) -> dict:
    async with ClientSession() as session:
      try:
        # 构造请求数据
        req = ProfileRequest(request_type="query_profile", timestamp=int(time.time()), version="1.0", data=ProfileData(uid=uid))
